...
while True:
    ...
    frame_counter += 1
    # Skip alternate frames for 2x speed boost on CPU
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            index_middle_dist = math.hypot(
                hand_landmarks.landmark[8].x - hand_landmarks.landmark[12].x,
            ...
            if not freeze_cursor and not full_palm and index_up and middle_up and index_middle_dist > 0.05:
                ...
                smooth_factor = 0.80
                screen_x = int(prev_screen_x * smooth_factor + target_x * (1 - smooth_factor))
                screen_y = int(prev_screen_y * smooth_factor + target_y * (1 - smooth_factor))
    ...
    if time.time() - click_text_time < TEXT_DURATION:
        cv2.putText(frame, click_text, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
